...
# load t_article and transport to [id, content]
def loadArticleData():
    connection = ConnDB(connectConfiguration)
    ...
    cursor.execute(sql)
    result = cursor.fetchall()
    result = [[x['id'], x['content']] for x in result]
    return result
    connection.close()

# clear data in t_similar
def clearSimilarTable():
    connection = ConnDB(connectConfiguration)
    cursor = connection.connect()
    cursor.execute("DROP TABLE IF EXISTS t_similar")
    sql = """CREATE TABLE `t_similar`  (
    `id` bigint UNSIGNED NOT NULL AUTO_INCREMENT,
    `article` bigint NOT NULL,
    `type` bigint NOT NULL,
    `create_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
    `update_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP(0),
    PRIMARY KEY (`id`) USING BTREE
    ) ENGINE = InnoDB CHARACTER SET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci ROW_FORMAT = Dynamic;"""
    cursor.execute(sql)
    connection.close()

# output data in t_similar
def outputSimilarData(article, type):
    connection = ConnDB(connectConfiguration)
    cursor = connection.connect()
    sql = "INSERT INTO t_similar (article, type) VALUES(%s, %s)"
    cursor.execute(sql, (article, type))
    connection.close()
